...
func setupRoutes(appServer *AppServer) *gin.Engine {
	...
	api := router.Group("/api/v1")
	{
		...
		api.POST("/publish_async", appServer.publishAsyncHandler)
		api.POST("/publish_video_async", appServer.publishVideoAsyncHandler)
		api.GET("/publish/jobs/:job_id", appServer.publishJobStatusHandler)
		api.GET("/publish/verify", appServer.verifyPublishedNoteHandler)
		api.POST("/publish/verify", appServer.verifyPublishedNoteHandler)
		api.POST("/publish", appServer.publishHandler)
		api.POST("/publish_video", appServer.publishVideoHandler)
		api.GET("/notes/recent", appServer.recentPublishedNotesHandler)
		api.POST("/notes/recent", appServer.recentPublishedNotesHandler)
